  ...
  return (
    <div className="relative group border border-cyan-700 p-6 cursor-pointer">
      ...
      <div className="flex items-start justify-between mt-4 space-x-4">
        <div className="font-semibold text-gray-900 sm:text-sm text-xs md:text-base">
          <p className="w-[100px] overflow-hidden text-ellipsis whitespace-nowrap">
            {singleProductTile?.title}
          </p>
        </div>
        <div className="text-right ">
          <p className="text-xs font-semibold text-gray-900 sm:text-sm md:text-[14px] ">
            {singleProductTile?.price}
          </p>
        </div>
      </div>
      <button className="px-5 mt-5 w-full py-2 rounded-none bg-black text-white font-semibold text-lg">
        View Details
      </button>
    </div>
  );
